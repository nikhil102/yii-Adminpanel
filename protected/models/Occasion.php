...
class Occasion extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name,uniquename, imglink, appimagelink, description', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('name, uniquename', 'length', 'max'=>100),
			array('imglink, description', 'length', 'max'=>400),
			array('appimagelink', 'length', 'max'=>300),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, uniquename, imglink, appimagelink, description, status', 'safe', 'on'=>'search'),
		);
	}
