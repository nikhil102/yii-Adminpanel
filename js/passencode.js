...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
function loginAction(form, key, user) {
    $j(".modal-backdrop").show();
    var password = jQuery("#ULoginForm_password").val();
    $j("#ULoginForm_password").val(Aes.Ctr.encrypt(password, key, 256));
    $j("#" + form).append("<input type='hidden' name='ULoginForm[login_token]' value='" + key + "'/>");
    //$j("#" + form).append("<input type='hidden' name='LoginForm[no_of_attempts]' value='" + no_of_attempts + "'/>");
    $j($j("#ULoginForm_password")).parents("form").submit();
}

/**
 * Method to initailize login actions
 */